...
function addFirst() {
    ...
    let newTask = generateRandomTask();
    let addition = `
    <div class="task">
        <span class="${newTask.isCompleted ? 'completed' : ''}">${newTask.text}</span> -
        <span class="status">${newTask.isCompleted ? 'completed' : 'pending'}</span>
    </div>
    `;
    element.innerHTML = addition + element.innerHTML
}
